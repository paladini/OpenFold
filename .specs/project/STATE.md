...
- [x] Verify current wry/tao crate versions and WebView2 bootstrap strategy at M6 start — done 2026-07-07: `wry = "0.55"` (0.55.1), `tao = "0.35"` (0.35.0), `rust-embed = "8.11"`, `directories = "6"`, `serde`/`serde_json = "1"`. `WebViewBuilder::with_custom_protocol(name, Fn(WebViewId, Request<Vec<u8>>) -> Response<Cow<'static,[u8]>>)` and `with_ipc_handler(Fn(Request<String>))` confirmed via docs.rs and validated by `cargo build`. Webview data-dir pinning is `WebViewBuilder::new_with_web_context(&mut WebContext::new(Some(path)))`, not a `with_data_directory` builder method (that name doesn't exist on this version) — found only by trial against the real API via `cargo build` after doc search came up empty.
- [x] M6 manual launch checklist (Windows dev machine, release build, 2026-07-07): binary 1.6 MB (budget < 10 MB); host process RSS ~20 MB idle for 5s (budget < 50 MB); window opens titled "OpenFold" at the spec'd 1280x800 default, centered (measured position matched primary-monitor centering math); data dir created at exact `%LOCALAPPDATA%\OpenFold` (instance.lock + webview/ pinned WebView2 profile); graceful close (WM_CLOSE via CloseMainWindow) triggers save and window-state.json persists correctly, process exits cleanly with no orphan. **Real end-to-end IPC roundtrip confirmed**, not just infrastructure: wired apps/web's main.tsx to call `desktopBridge.invoke('ping')` once on load (no-op in a browser), and the Rust host to print `"openfold: ready"` the first time that ping actually routes -- launched the release binary, polled stdout, saw the marker at **720ms** (budget < 2s), proving the full pipeline (window -> webview -> openfold:// custom protocol asset serving -> real SPA JS execution -> window.ipc.postMessage -> ipc::route -> evaluate_script callback) genuinely works, not merely that its pieces compile. **Not verified this session** (no native-window GUI automation tool, browser-preview tools only work on browser tabs): visual inspection of rendered SPA content, IndexedDB round-trip across an actual relaunch, offline-launch behavior, second-instance focus-and-exit, and macOS/Linux entirely (Windows-only dev machine). Flagged honestly rather than claimed.
